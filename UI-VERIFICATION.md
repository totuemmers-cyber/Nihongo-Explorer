...
- `npm run test:storage`: covers denied storage getters, reads and writes; startup, theme/sound controls, bookmark toggling and malformed bookmark data, learning progress, normal persistence, and in-memory fallback. A visible notice explains that unsaved changes may be lost on reload or close.
